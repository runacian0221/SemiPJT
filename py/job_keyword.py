...
class JobKeywords:
    def __init__(self, data_path):
        ...
        self.keywords_df = pd.DataFrame(columns=['job', 'keywords', '자소서갯수'])

    # 데이터 불러오고 기업별 자소서 갯수가 10개 미만인 기업 제외
    def load_data(self):
        self.df = pd.read_csv(self.data_path)
        counts = self.df['job'].value_counts()
        under10 = counts[counts<10].index
        self.df = self.df.drop(self.df[self.df['job'].isin(under10)].index)

    # 기업별로 자기소개서 그룹화
    def group_data(self):
        self.grouped_df = self.df.groupby('job')['answer'].apply(' '.join).reset_index()
        self.content_counts = self.df['job'].value_counts().reset_index()
        self.content_counts.columns = ['job', '자소서갯수']
    ...
    def calculate_tfidf(self):
        ...
        for idx, row in self.grouped_df.iterrows():
            ...
            content_count = self.content_counts.loc[self.content_counts['job'] == job, '자소서갯수'].values[0]
            self.keywords_df = self.keywords_df.append({'job': job, 'keywords': top_5_keywords, '자소서갯수': content_count}, ignore_index=True)
...
if __name__ == "__main__":
    extractor = JobKeywords('csv/jobkorea_question_labeled_mecab.csv')
    extractor.run('job_keywords.csv')
